...
const registerShiprocketPickupLocation = async (req, res) => {
  ...
  try {
    ...
    if(data.status_code === 422) return res.status(422).json({status : false,  message: "A Pickup Address is already exist in the system with this name. Please use a different name." });
