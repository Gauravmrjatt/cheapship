...
export const mobileRegSchema = z.object({
  mobile: z.string().min(10, "Mobile number must be at least 10 digits").regex(/^\d+$/, "Must be only digits"),
});

export const otpRegSchema = z.object({
  otp: z.string().length(6, "OTP must be 6 digits"),
});

export const detailsRegSchema = z
  .object({
    name: z.string().min(1, "Name is required"),
