...
export default function AdminForgotPasswordPage() {
  ...
  const sendOtpMutation = useMutation<SendOtpResponse, Error, ForgotPasswordFormData>(
    http.post<SendOtpResponse, ForgotPasswordFormData>("/admin/forgot-password", {
      onSuccess: (data) => {
        setStep(2);
  ...
  const resetPasswordMutation = useMutation<{ message: string }, Error, ResetPasswordFormData>(
    http.post<{ message: string }, ResetPasswordFormData>("/admin/reset-password", {
      onSuccess: (data) => {
  ...
  const stepDescriptions = {
    1: "Enter your admin email to receive a password reset OTP.",
  ...
  return (
    <div className="min-h-screen flex items-center justify-center">
      <Card className="w-full max-w-sm">
        <CardHeader>
          <CardTitle className="text-2xl">Admin Reset Password</CardTitle>
          ...
          <form onSubmit={emailForm.handleSubmit(onSendOtp)}>
            <CardContent className="grid gap-4">
              <Field>
                ...
                <Input placeholder="admin@example.com" type="email" {...emailForm.register("email")} />
